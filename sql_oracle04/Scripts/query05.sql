...
SELECT * FROM EMPLOYEES WHERE EMPLOYEE_ID IN (208, 211);
...
/*
 * DEPARTMENTS 테이블에서 MANAGER_ID 가 없는 부서는 삭제하도록 한다.
 * NOTICE 테이블도 삭제할 부서의 공지사항이 삭제되도록 한다.
 */
DELETE
  FROM NOTICE
 WHERE DEPT_ID IN (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE MANAGER_ID IS NULL);

DELETE
  FROM DEPARTMENT
 WHERE MANAGET_ID IS NULL;

/*
 * EMPLOYEES 테이블의 COMMISSION_PCT 가 NULL 인 경우 0 으로 수정 한다.
 */
UPDATE EMPLOYEES
 ...
 * 해당 부서의 MANAGER_ID 값이 EMPLOYEES 테이블의 MANAGER_ID 가 되도록 수정 한다.
 */
UPDATE EMPLOYEES E
 ...
 * 해당 사원의 DEPARTMENT_ID 값이 EMPLOYEES 테이블의 DEPARTMENT_ID 가 되도록 수정 한다.
 */
UPDATE EMPLOYEES E1
