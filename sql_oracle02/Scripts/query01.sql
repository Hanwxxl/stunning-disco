/*
 * DDL
 *     - 데이터 정의어로 데이터를 저장하고 다루기 위한 위한 객체를
 ...
 *     PRIMARY KEY : NULL, 중복값 저장을 모두 허용하지 않음(Record 의 고유 식별값으로 사용하기 위해)
 ...
 * 
 * 위의 제약 조건을 위반하는 데이터 추가/수정/삭제 작업이 이루어 지는 경우 "제약조건 위반" 이라는 에러가
